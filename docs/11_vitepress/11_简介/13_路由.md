...
[Getting Started](https://vitepress.dev/zh/getting-started)
[Getting Started](https://vitepress.dev/zh/guide/getting-started)

<!-- Don't -->

[Getting Started](https://vitepress.dev/zh/getting-started.md)
[Getting Started](https://vitepress.dev/zh/getting-started.html)
```

在[资源处理](https://vitepress.dev/zh/asset-handling)中了解有关链接到资源（例如图像）的更多信息。
...
可以通过像这样配置 [`rewrites`](https://vitepress.dev/zh/reference/site-config#rewrites) 选项来实现此目的：
...
```js
export default {
  paths: () => [{ params: { pkg: 'foo', version: '1.0.0' } }, { params: { pkg: 'foo', version: '2.0.0' } }, { params: { pkg: 'bar', version: '1.0.0' } }, { params: { pkg: 'bar', version: '2.0.0' } }]
}
```
...
还可以通过 [`useData`](https://vitepress.dev/zh/reference/runtime-api#usedata) 运行时 API 访问当前页面的参数。这在 Markdown 文件和 Vue 组件中都可用：
