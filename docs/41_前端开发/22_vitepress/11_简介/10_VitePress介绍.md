...
只是想尝试一下？跳到[快速开始](https://vitepress.dev/zh/getting-started)。
...
- **博客、档案和营销网站**
  ...
  VitePress 支持[完全的自定义主题](https://vitepress.dev/zh/custom-theme)，具有标准 Vite + Vue 应用程序的开发体验。基于 Vite 构建还意味着可以直接利用其生态系统中丰富的 Vite 插件。此外，VitePress 提供了灵活的 API 来[加载数据](https://vitepress.dev/zh/data-loading) (本地或远程)，也可以[动态生成路由](https://vitepress.dev/zh/routing#dynamic-routes)。只要可以在构建时确定数据，就可以使用它来构建几乎任何东西。
...
- **[内置 Markdown 扩展](https://vitepress.dev/zh/markdown)**：frontmatter、表格、语法高亮……应有尽有。具体来说，VitePress 提供了许多用于处理代码块的高级功能，使其真正成为技术文档的理想选择。

- **[Vue 增强的 Markdown](https://vitepress.dev/zh/using-vue)**：每个 Markdown 页面都是 Vue [单文件组件](https://cn.vuejs.org/guide/scaling-up/sfc.html)，这要归功于 Vue 模板与 HTML 的 100% 语法兼容性。可以使用 Vue 模板语法或导入的 Vue 组件在静态内容中嵌入交互性。
