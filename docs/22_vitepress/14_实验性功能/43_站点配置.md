...
配置文件总是从 `<root>/.vitepress/config.[ext]` 解析，其中 `<root>` 是 VitePress [项目根目录](https://vitepress.dev/zh/guide/routing#root-and-source-directory)，`[ext]` 是支持的文件扩展名之一。开箱即用地支持 TypeScript。支持的扩展名包括 `.js`、`.ts`、`.mjs` 和 `.mts`。
...
- 类型：`string`
- 默认值： `VitePress`
- 每个页面可以通过 [frontmatter](https://vitepress.dev/zh/frontmatter-config#title) 覆盖
...
- 类型：`string | boolean`
- 每个页面可以通过 [frontmatter](https://vitepress.dev/zh/frontmatter-config#titletemplate) 覆盖
...
- 每个页面可以通过 [frontmatter](https://vitepress.dev/zh/frontmatter-config#description) 覆盖
...
- 类型：`HeadConfig[]`
- 默认值： `[]`
- 可以通过 [frontmatter](https://vitepress.dev/zh/frontmatter-config#head) 为每个页面追加
...
当设置为 `true` 时，VitePress 将从 URL 中删除 `.html` 后缀。另请参阅[生成简洁的 URL](https://vitepress.dev/zh/guide/routing#generating-clean-url)。
...
自定义目录 &lt;-&gt; URL 映射。详细信息请参阅[路由：路由重写](https://vitepress.dev/zh/guide/routing#route-rewrites)。
...
相对于项目根目录的 markdown 文件所在的文件夹。另请参阅[根目录和源目录](https://vitepress.dev/zh/guide/routing#root-and-source-directory)。
...
- 类型：`string`
- 默认值： `https://vitepress.dev/zh/.vitepress/dist`

项目的构建输出位置，相对于[项目根目录](https://vitepress.dev/zh/guide/routing#root-and-source-directory)。
...
缓存文件的目录，相对于[项目根目录](https://vitepress.dev/zh/guide/routing#root-and-source-directory)。另请参阅：[cacheDir](https://vitejs.dev/config/shared-options.html#cachedir)。
...
设置为 `true` 时，生产应用程序将在 [MPA 模式](https://vitepress.dev/zh/guide/mpa-mode)下构建。MPA 模式默认提供 零 JavaScript 支持，代价是禁用客户端导航，并且需要明确选择加入才能进行交互。
...
是否使用 Git 获取每个页面的最后更新时间戳。时间戳将包含在每个页面的页面数据中，可通过 [`useData`](https://vitepress.dev/zh/runtime-api#usedata) 访问。

使用默认主题时，启用此选项将显示每个页面的最后更新时间。可以通过 [`themeConfig.lastUpdatedText`](https://vitepress.dev/zh/default-theme-config#lastupdatedtext) 选项自定义文本。
...
export default {
  transformPageData(pageData) {
    const canonicalUrl = `https://example.com/${pageData.relativePath}`.replace(/index\.md$/, '').replace(/\.md$/, '.html')
