...
将配置的 [`base`](https://vitepress.dev/zh/site-config#base) 追加到给定的 URL 路径。另请参阅 [Base URL](https://vitepress.dev/zh/guide/asset-handling#base-url)。

## `<Content />` <Badge type="info" text="component" />

`<Content />` 组件显示渲染的 markdown 内容。在[创建自己的主题时](https://vitepress.dev/zh/guide/custom-theme)很有用。
...
在 Vue 表达式中直接访问当前页面的 [frontmatter](https://vitepress.dev/zh/guide/frontmatter) 数据。
...
在 Vue 表达式中直接访问当前页面的[动态路由参数](https://vitepress.dev/zh/guide/routing#dynamic-routes)。
