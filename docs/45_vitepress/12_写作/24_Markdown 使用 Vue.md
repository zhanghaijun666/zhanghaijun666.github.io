...
::: tip SSR 兼容性
所有的 Vue 用法都需要兼容 SSR。参见 [SSR 兼容性](https://vitepress.dev/zh/ssr-compat)获得更多信息和常见的解决方案。
:::
...
如果一个组件要在大多数页面上使用，可以通过自定义 Vue 实例来全局注册它们。有关示例，请参见[扩展默认主题](https://vitepress.dev/zh/extending-default-theme#registering-global-components)中的相关部分。
...
::: details
<<< @/45_vitepress/components/ModalDemo.vue
:::
