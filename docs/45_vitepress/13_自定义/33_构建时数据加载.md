...
该辅助函数接受一个相对于[源目录](https://vitepress.dev/zh/routing#source-directory)的 glob 模式，并返回一个 `{ watch, load }` 数据加载对象，该对象可以用作数据加载文件中的默认导出。它还基于文件修改时间戳实现了缓存以提高开发性能。
...
`createContentLoader` API 也可以在[构建钩子](https://vitepress.dev/zh/reference/site-config#build-hooks)中使用：
