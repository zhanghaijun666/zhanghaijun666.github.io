...
export function setItem(cwd: string, list: string[], options: Options, cache: Cache = {}, link = ''): Item | undefined {
  ...
  if (!isFile) {
    ...
    // 设置分组名
    if (groupTitle) text = groupTitle

    if (groupIndex) index = groupIndex // 设置分组索引
 ...
 * @returns 侧边栏数据
 */
export function setDataFormat(cwd: string, paths: string[], { sort = () => 0, ...options }: Options, cache: Cache = {}): Item[] {
  let root: Item[] = []

  const list = useSortIndexName(paths)

  list.forEach((path) => {
    const obj = setItem(cwd, path.split(sep), options, cache)!
  ...
  function deep(list: Item[], obj: Item, root: Item[]) {
    ...
    if (node) {
      ...
      // Group 分组提升至对应顶层
      if (obj.group && !obj.isFile) root.push(obj)
      else list.push(obj)
    }
