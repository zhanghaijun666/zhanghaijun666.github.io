...
export function isColor(value: string): boolean {
  ...
  if (value === 'transparent' || NAMED_COLORS[value as ColorName]) return true;
...
export function parseStringColor(color: string) {
  ...
  if (color === 'transparent') return { r: 0, g: 0, b: 0, a: 0, format: 'name', toString: toRgbString } as ObjectColor;
...
export function parseColorToRgba(originColor: Color) {
  ...
  if (typeof originColor === 'string') color = parseStringColor(originColor);
  else color = originColor;

  if (color !== null && typeof color === 'object') {
    if ('l' in color) rgb = hslToRgb(color.h, color.s, color.l);
    else if ('v' in color) rgb = hsvToRgb(color.h, color.s, color.v);
...
export function rgbToHex(r: number | string, g: number | string, b: number | string, allow3Char = false) {
  ({ r, g, b } = normalizeRgb(r, g, b));

  const hex = [repairDigits(Math.round(r * 255).toString(16)), repairDigits(Math.round(g * 255).toString(16)), repairDigits(Math.round(b * 255).toString(16))];

  if (allow3Char && isRepetitive(hex[0]) && isRepetitive(hex[1]) && isRepetitive(hex[2])) return hex[0].charAt(0) + hex[1].charAt(0) + hex[2].charAt(0);

  return `#${hex.join('')}`;
}

export function rgbaToHex(r: number | string, g: number | string, b: number | string, a: number | string, allow4Char = false) {
  ({ r, g, b } = normalizeRgb(r, g, b));
  ...
  if (allow4Char && isRepetitive(hex[0]) && isRepetitive(hex[1]) && isRepetitive(hex[2]) && isRepetitive(hex[3]))
...
export function randomColor(withAlpha = false, type: 'hex' | 'rgb' | 'hsv' | 'hsl' = 'hex') {
  ...
  if (type === 'hex') return withAlpha ? rgbaToHex(r, g, b, Math.random()) : rgbToHex(r, g, b);

  let color: ObjectColor;

  if (type === 'hsl') color = rgbToHsl(r, g, b);
  else if (type === 'hsv') color = rgbToHsv(r, g, b);
  else color = { r, g, b };

  if (withAlpha) (color as RGBAColor).a = Math.random();
...
function toRgbString(this: RGB) {
  if (isDefined(this.a) && this.a >= 0 && this.a < 1) return `rgba(${this.r}, ${this.g}, ${this.b}, ${this.a})`;

  return `rgb(${this.r}, ${this.g}, ${this.b})`;
}

function toHslString(this: HSL) {
  if (isDefined(this.a) && this.a >= 0 && this.a < 1) return `hsla(${this.h}, ${this.s}, ${this.l}, ${this.a})`;

  return `hsl(${this.h}, ${this.s}, ${this.l})`;
}

function toHsvString(this: HSV) {
  if (isDefined(this.a) && this.a >= 0 && this.a < 1) return `hsva(${this.h}, ${this.s}, ${this.v}, ${this.a})`;
...
export function computeSeriesColors(value: Color) {
  ...
  const black = parseColorToRgba({ r: 0, g: 0, b: 0, a: 1 });
  const white = parseColorToRgba({ r: 255, g: 255, b: 255, a: 1 });
