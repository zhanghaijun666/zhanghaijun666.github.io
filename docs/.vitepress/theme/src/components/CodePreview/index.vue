    ...
    <section :class="[ns.bem('description')]">
      ...
      <div :class="[ns.bem('description', 'handle-btn')]">
        <el-tooltip content="查看源代码" :show-arrow="false">
          <el-icon :size="16" class="op-btn" @click="setCodeFold(!isCodeFold)">
            <View />
          </el-icon>
        </el-tooltip>
        <el-tooltip content="复制代码" :show-arrow="false">
          <el-icon :size="16" class="op-btn" @click="clickCodeCopy">
...
import { useNameSpace } from '@haijunit/utils/src/use-namespaces';
import { computed, onMounted, ref, watch } from 'vue';

const ns = useNameSpace();
const props = withDefaults(defineProps<{ code: string; showCode: string; title: string; description: string }>(), {
  title: '默认标题',
  description: '描述内容',
});
// 代码折叠
const isCodeFold = ref<boolean>(true);
const setCodeFold = (value: boolean) => {
  isCodeFold.value = value;
};
const clickCodeCopy = () => {
  console.log('复制成功', sourceCode.value);
};
