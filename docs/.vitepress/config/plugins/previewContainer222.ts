...
export const parseContainerTag = (md: MarkdownIt) => {
  ...
  md.renderer.rules.container_preview_open = (tokens: Token[], idx: number, options: MarkdownIt.Options, env: any, self: Renderer) => {
    const token = tokens[idx];
    // 组件的相对路径
    const componentRelativePath = isCheckingRelativePath(tokens[idx + 2].content.trim());
    // 组件绝对路径
  ...
  md.renderer.rules.container_preview_close = (tokens: Token[], idx: number, options: MarkdownIt.Options, env: any, self: Renderer) => {
    const token = tokens[idx];
